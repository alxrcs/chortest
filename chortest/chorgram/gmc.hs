...
{-
	Extract the machines from a file and generate the dot file
	containing the CFSMs and their corresponging transition
	system(s).
-}
    ...
    then do putStrLn $ usage GMC
               ...
               let sys' =
                     case (flags!"-D") of
                       ...
                       "no"  -> sys
                       _     ->
                         error ((flags!"-D") ++ " unknown; for flag -D; use \"min\", \"det\", or \"no\"" )
               ...
               writeToFile (dir ++ ".machines")
                 (rmChar '\"' $ show $ L.foldr (\x y -> x ++ (if y=="" then "" else " ") ++ y) "" (cfsmsIds system))
               let bufferSize =
                     read (flags ! "-b") :: Int
               ...
               myPrint flags GMC (
                 "Synchronous TS:\t(nodes " ++ (show $ (S.size $ gnodes ts0)) ++
                 ", transitions " ++ (show $ (S.size $ edgesOf ts0)) ++ ")"
                 )
               when (bufferSize > 0) $
                 myPrint flags GMC (
                   (flags ! "-b") ++
                   "-bounded TS:\t(nodes " ++ (show $ (S.size $ gnodes tsb)) ++
                   ", transitions " ++ (show $ (S.size $ edgesOf tsb)) ++")"
                   )
               if flags!"-ts" == "ts"
                 then do
