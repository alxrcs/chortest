...
isCommuting ts@(_, _, _, trans) n e1 e2 =
  let
    ...
    n2 = succConf ts n e2
    aux =
      \x -> S.member (n1,e2,x) trans && S.member (n2,e1,x) trans
    intersection =
      S.intersection (succConfs ts n1) (succConfs ts n2)
  in
    F.or $ S.map aux intersection
...
buildLessRel ts@(nodes, initnode, _, _) =
    ...
    where
      helper border acc visited =
        let
          aux n =
            S.map (\x -> (n,x)) (S.filter (\y -> not $ S.member y visited) (succConfs ts n))
          newpairs =
        ...
        in
          ...
          then
            ...
            helper newnodes (S.union acc newpairs) (S.union newnodes visited)
              
 
fire :: TSb -> Configuration -> KEvent -> Bool
fire (_, _, _, tstrans) n e =
  not $ S.null $ S.filter (\(x,y,_) -> x==n && y == e) tstrans
...
isLastNodeRel ts rel n e1 e2 =
  (F.and $ S.map checkNode (succConfs ts n))
  where
    checkNode n' =
      not ((fire ts n' e1) &&
           (fire ts n' e2) &&
...
checkBranchingProperty sys ts@(nodes, _, _, _) =
...
checkNodes mysys ts lessRel nodes =
  L.concat $ L.map checkNode nodes
  where
    checkNode n =
      checkBranch [] n (pairs $ S.toList $ (succEvents ts n))
    pairs xs =
      L.nubBy eqTest [(x,y) | x <- xs, y <- xs, x /= y]
    eqTest (x,y) (x',y') =
      ((x == y') && (y == x')) || ((x == x') && (y == y'))
    checkBranch res _ [] =
      res
    checkBranch res n ((e1, e2):evs) =
      let
        oneSender =
          uniqueSender mysys ts n e1 hdL1 e2 hdL2
        norace =
          noRace (sender e1) mysys ts hdL1 hdL2 ids n e1 e2
        chAwareness =
          choiceAwareness ts ids n e1 hdL1 e2 hdL2
        setMsg b y =
          if b
          then ""
          else y
        msg1 =
          setMsg oneSender "Not unique sender"
        msg2 =
          setMsg norace "There is a race"
        msg3 =
          setMsg chAwareness "No choice awareness"
        analysis =
          if oneSender && chAwareness && norace
          then []
          else [Rp n e1 e2 (msg1 ++ " " ++ msg2 ++ " " ++ msg3)]
        ids =
          cfsmsIds mysys
        (hdL1, hdL2) =
          (mapActions mysys ts (succConf ts n e1) e1,
           mapActions mysys ts (succConf ts n e2) e2
          )
      in
        if (independent e1 e2) || (isCommuting ts n e1 e2) || (not (isLastNodeRel ts lessRel n e1 e2))
        then
          checkBranch res n evs
        else
          checkBranch (res ++ analysis) n evs
...
mapActions mysys ts n e =
  M.fromList $ L.map (\p -> (p, actsOf p)) (cfsmsIds mysys)
  where
    actsOf p =
      case project e p of
  ...
  let
    senders =
      L.map
        ...
        (cfsmsIds mysys)
    checkSenders num (x:xs) =
      checkSenders (if x then num+1 else num) xs
    checkSenders num [] =
      (num == 1)
  in
    checkSenders 0 senders
    ...
    check q e e' =
      let
        (lt, lt') =
          (hdL1 ! q, hdL2 ! q)
        (lt1, lt2) =
          (S.map action2interaction lt, S.map action2interaction lt')
        (succ, succ') =
          (succConf ts n e, succConf ts n e')
        sendOK =
          (not $ existSend $ S.union lt lt') && (not $ sameSenders lt lt')
        compatible =
          compatibleActions lt lt'
      in
        if sendOK && compatible
        then
          (dependencyTS s p ts succ e lt1 lt2) &&
          (dependencyTS s p ts succ' e' lt2 lt1)
        else True
  in
    if (check p e1 e2)
  ...
  let
    ...
    isAware q n' e e' =
      let
        ...
        lt2 = hdL2 ! q
      in
        (compatibleActions lt1 lt2) || (goodMergeNode ts q n' succ1 e succ2 e')
  in
    if (isAware p n e1 e2) 
    then
      (choiceAwareness ts ps n e1 hdL1 e2 hdL2)
    else
      False
{-      
      error $
